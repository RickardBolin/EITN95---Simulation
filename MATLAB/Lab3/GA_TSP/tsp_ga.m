    ...
    for iter = 1:numIter
        ...
        for p = 4:4:popSize
            ...
            if rand(1) < 1/20
                ...
                tmpPop = zeros(4, n);
                for k = 1:4     
                    % Find 10 consequtive indexes between 0 and 39
                    random_crossover = randi(40);
                    random_parents = randi([1, n], 2);
                    % Initially, set the offspring to the second parent
                    offspring = pop(random_parents(2), :);
                    parent_1 = pop(random_parents(1), :);
                    % Find the cities at the indexes in the first parent
                    cities = parent_1(random_crossover:random_crossover+crossovers-1);
                    parent_2 = pop(random_parents(2), :);
                    idx_in_parent_2 = zeros(crossovers, 1);
                    % Find the indexes those cities has in parent 2
                    for i=1:crossovers
                        idx_in_parent_2(i) = find(parent_2 == cities(i));
                    end
                    % Set the offsprings' cities at those indexes to 0
                    offspring(idx_in_parent_2) = 0;
                    % Remove all zeros
                    temp_new_offspring = nonzeros(offspring);
                    % Insert the chunk form parent one into the offspring
                    new_offspring = [temp_new_offspring(1:random_crossover); cities'; temp_new_offspring(random_crossover+1:end)];
                    tmpPop(k, :) = new_offspring'; 
                end 
